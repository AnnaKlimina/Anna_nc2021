...
async function checkLogin(event) {
  ...
  let response = await fetch("../../../server/index.php?checkLogin=" + login);
  let check;
  if (response.ok) {
...
form.submit.addEventListener("click", async function (event) {
  ...
  if (
    ...
    user.avatar = form.avatar.files[0]
      ...
      : "./png/defaultUserAvatar.png";
    user.library = "";
    let formData = new FormData();
    formData.append(
      "addUser",
      JSON.stringify([user.login, user.password, user.mail, user.avatar])
    );
    let response = await fetch("../../../server/index.php?addUser", {
      method: "POST",
      mode: "cors",
      dataType: "json",
      headers: {
...
signInForm.submit.addEventListener("click", async function (event) {
  event.preventDefault();
  let response = await fetch(
    "../../../server/index.php?authorize&login=" +
...
async function signOut() {
  ...
  switch (document.querySelector(".main-page").dataset.currentPage) {
    ...
    case "user-library-page":
      let response = await fetch("../../../server/index.php?library");
      let library;
      if (response.ok) {
